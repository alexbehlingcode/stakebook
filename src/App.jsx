...
const S = {
  app:{fontFamily:T.body,background:T.bg,color:T.text,minHeight:"100vh",fontSize:"14px",lineHeight:1.5,overflowX:"hidden",width:"100%"},
  maxW:{maxWidth:1200,margin:"0 auto",padding:"0 20px",width:"100%",boxSizing:"border-box"},
  header:{background:"rgba(255,255,255,0.88)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${T.border}`,padding:"14px 0",paddingTop:"max(14px, env(safe-area-inset-top))",position:"sticky",top:0,zIndex:100},
  hInner:{maxWidth:1200,margin:"0 auto",padding:"0 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,width:"100%",boxSizing:"border-box"},
  ...
  sec:{padding:"24px 0"},
  card:{background:T.card,borderRadius:T.r,border:`1px solid ${T.border}`,padding:24,boxShadow:T.shadow,boxSizing:"border-box",overflow:"hidden"},
  ...
  input:{width:"100%",padding:"10px 12px",borderRadius:T.rs,border:`1px solid ${T.border}`,fontSize:16,fontFamily:T.body,background:"#FAFAF7",outline:"none",boxSizing:"border-box",transition:"border 0.2s"},
  select:{width:"100%",padding:"10px 12px",borderRadius:T.rs,border:`1px solid ${T.border}`,fontSize:16,fontFamily:T.body,background:"#FAFAF7",outline:"none",boxSizing:"border-box",appearance:"none",cursor:"pointer"},
...
function AuthScreen({ onAuth }) {
  ...
  const [password, setPassword] = useState(() => localStorage.getItem("sb_pw") ? atob(localStorage.getItem("sb_pw")) : "");
  ...
  useEffect(() => {
    const tryFaceID = async () => {
      if (localStorage.getItem("sb_biometric") !== "true") return;
      const savedEmail = localStorage.getItem("sb_email");
      const savedPw = localStorage.getItem("sb_pw");
      if (!savedEmail || !savedPw) return;
      const ok = await runBiometricAuth("Sign in to Stakebook");
      if (ok) {
        setLoading(true);
        const { error } = await supabase.auth.signInWithPassword({ email: savedEmail, password: atob(savedPw) });
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        if (rememberMe) { localStorage.setItem("sb_email", email); localStorage.setItem("sb_pw", btoa(password)); }
        else { localStorage.removeItem("sb_email"); localStorage.removeItem("sb_pw"); }
      } else if (mode === "signup") {
        ...
        <div style={{ ...S.card, padding: 28 }}>
          ...
          {mode === "login" && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <input type="checkbox" id="rememberMe" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} style={{ accentColor: T.brand, width: 16, height: 16, cursor: "pointer" }} />
...
function AppMain({ user }) {
  ...
  const filtered = useMemo(() => { if (tp === "today") return bets.filter(b => b.date === todayStr); if (tp === "yesterday") return bets.filter(b => b.date === yesterdayStr); if (tp === "thisweek") { const now = new Date(); const dow = now.getDay(); const off = dow === 0 ? 6 : dow - 1; const mon = new Date(now.getFullYear(), now.getMonth(), now.getDate() - off); const monStr = `${mon.getFullYear()}-${String(mon.getMonth()+1).padStart(2,"0")}-${String(mon.getDate()).padStart(2,"0")}`; return bets.filter(b => b.date >= monStr); } const c = getCutoff(tp); return bets.filter(b => new Date(b.date) >= c); }, [bets, tp]);
  ...
  const tpBarRef = useRef(null);
  const TPBar = () => {
    const localRef = useRef(null);
    useEffect(() => {
      if (localRef.current && tpBarRef.current !== null) {
        localRef.current.scrollLeft = tpBarRef.current;
      }
    });
    const handleScroll = (e) => { tpBarRef.current = e.target.scrollLeft; };
    return (
    <div ref={localRef} onScroll={handleScroll} style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch", width: "100%", paddingBottom: 4, scrollbarWidth: "none" }}>
  ...
  return (
    <div style={{...S.app, paddingBottom: mob ? 80 : 0}}>
      ...
      {mob && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(16px)", borderTop: `1px solid ${T.border}`, zIndex: 100, padding: "6px 0", paddingBottom: "max(6px, env(safe-area-inset-bottom))", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
        {[{ id: "dashboard", l: "Home", i: "chart" }, { id: "bets", l: "Bets", i: "book" }, { id: "analytics", l: "Stats", i: "trend" }, { id: "health", l: "Health", i: "shield" }, { id: "account", l: "Account", i: "user" }].map(t => (
      ...
      {/* ═══ VIEW MODAL ═══ */}
      {viewing && (
        <div style={S.overlay} onClick={() => setViewing(null)}><div style={{...S.modal, ...(mob ? {width:'100%',maxWidth:'100%',height:'100vh',maxHeight:'100vh',borderRadius:0,padding:20,paddingTop:'max(20px, env(safe-area-inset-top))'} : {})}} onClick={e => e.stopPropagation()}>
      ...
      {!mob && (
        <footer style={{ borderTop: `1px solid ${T.border}`, marginTop: 48, padding: "24px 0", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", gap: 24 }}>
            <a href="https://www.stakebook.live/terms.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: T.sub, textDecoration: "none" }}>Terms of Service</a>
            <a href="https://www.stakebook.live/privacy.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: T.sub, textDecoration: "none" }}>Privacy Policy</a>
            <a href="https://www.stakebook.live/disclaimer.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: T.sub, textDecoration: "none" }}>Disclaimer</a>
          </div>
          <div style={{ fontSize: 11, color: T.light, marginTop: 8 }}>&copy; 2026 Stakebook</div>
        </footer>
      )}
    </div>
  );
...
function BetModal({ bet, isEdit, settings, saving, onSave, onClose }) {
  ...
  return (
    <div style={S.overlay} onClick={onClose}><div style={{...S.modal, ...(mob ? {width:'100%',maxWidth:'100%',height:'100vh',maxHeight:'100vh',borderRadius:0,padding:20,paddingTop:'max(20px, env(safe-area-inset-top))',overflowX:'hidden',overflowY:'auto'} : {})}} onClick={e => e.stopPropagation()}>
      ...
      <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: mob ? 10 : 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
          <div><label style={S.label}>Date</label><input type="date" style={{...S.input, fontSize: 16, width: "100%", boxSizing: "border-box", WebkitAppearance: "none", appearance: "none", paddingRight: 8}} value={f.date} onChange={e => up("date", e.target.value)} /></div>
          <div><label style={S.label}>Sport{f.betType === "Parlay" ? "s" : ""}</label>
              {f.betType === "Parlay" ? (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "8px 0" }}>
                  {SPORTS.filter(s => s !== "All").map(s => {
                    const selected = (f.sport || "").split(",").map(x => x.trim()).filter(Boolean);
                    const isOn = selected.includes(s);
                    return (
                      <button key={s} type="button" onClick={() => {
                        const cur = (f.sport || "").split(",").map(x => x.trim()).filter(Boolean);
                        const next = isOn ? cur.filter(x => x !== s) : [...cur, s];
                        up("sport", next.join(", "));
                      }} style={{ padding: "5px 10px", borderRadius: 20, border: `1px solid ${isOn ? T.brand : T.border}`, background: isOn ? T.brandLight : "transparent", color: isOn ? T.brand : T.sub, fontSize: 13, fontWeight: isOn ? 600 : 400, cursor: "pointer", fontFamily: T.body }}>
                        {s}
                      </button>
                    );
                  })}
                ...
                <select style={{...S.select, fontSize: 16}} value={f.sport} onChange={e => up("sport", e.target.value)}>{SPORTS.map(s => <option key={s}>{s}</option>)}</select>
              )}
            </div>
        </div>
...
async function runBiometricAuth(reason) {
  try {
    const { BiometricAuth } = await import("@aparajita/capacitor-biometric-auth");
...
function AccountScreen({ onLogout }) {
  ...
  const [biometricEnabled, setBiometricEnabled] = useState(() => localStorage.getItem("sb_biometric") === "true");
  ...
  const toggleBiometric = async () => {
    if (!biometricEnabled) {
      ...
      if (ok) { localStorage.setItem("sb_biometric", "true"); setBiometricEnabled(true); }
    } else {
      localStorage.removeItem("sb_biometric");
  ...
  return (
    <div style={S.sec}>
      <h2 style={{ fontFamily: T.display, fontSize: 24, fontWeight: 400, margin: "0 0 24px", fontStyle: "italic" }}>Account</h2>
      <div style={{ ...S.card, padding: 24, marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: biometricAvailable ? 20 : 0 }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: T.brand, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <I n="user" s={22} c="#fff" />
          </div>
          <div>
            <div style={{ fontWeight: 600, fontSize: 15, color: T.text }}>{userEmail}</div>
            <div style={{ fontSize: 12, color: T.sub, marginTop: 2 }}>Stakebook Member</div>
          </div>
        </div>
        {biometricAvailable && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0 0", borderTop: `1px solid ${T.border}` }}>
            <div>
              <div style={{ fontWeight: 600, fontSize: 14, color: T.text }}>Face ID Login</div>
              <div style={{ fontSize: 12, color: T.sub, marginTop: 2 }}>Sign in faster with Face ID</div>
            </div>
            <button onClick={toggleBiometric} style={{
              width: 44, height: 26, borderRadius: 13, border: "none", cursor: "pointer",
              background: biometricEnabled ? T.brand : T.border, position: "relative", transition: "background 0.2s", flexShrink: 0
            }}>
              <div style={{
                width: 20, height: 20, borderRadius: "50%", background: "#fff",
                position: "absolute", top: 3, transition: "left 0.2s",
                left: biometricEnabled ? 20 : 3, boxShadow: "0 1px 4px rgba(0,0,0,0.2)"
              }} />
            </button>
          </div>
        )}
      </div>
      <button onClick={onLogout} style={{ ...S.btn("ghost"), width: "100%", padding: "12px 20px", color: T.red, border: `1px solid ${T.red}22` }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 20 }}>
        ...
        <button onClick={() => openBrowser("https://www.stakebook.live/disclaimer.html")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: T.sub }}>Disclaimer</button>
      </div>
    </div>
  );
}
